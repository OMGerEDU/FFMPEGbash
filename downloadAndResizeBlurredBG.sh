...
# Resize the video using ffmpeg (CPU encoding with libx264 for speed)
TEMP_FILE="$(dirname "$OUTPUT_FILE")/temp.mp4"
ffmpeg -y -i "$OUTPUT_FILE" -filter_complex "[0:v]scale=1080:1920,boxblur=5:2[bg];[0:v]scale=-1:1080:force_original_aspect_ratio=decrease[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2" \
-c:v libx264 -preset veryfast -crf 23 -pix_fmt yuv420p -c:a copy -movflags +faststart "$TEMP_FILE"
